...
<details>
  ...
  | `AD_BUFFERING`               | Android, iOS | Fires when the ad has stalled playback to buffer.                                                                                                                                                           |
  | `AD_CAN_PLAY`                | iOS          | Fires when the ad is ready to play without buffering, either at the beginning of the ad or after buffering completes.                                                                                       |
  ...
  | `AD_PROGRESS`                | Android, iOS | Fires when the ad's current time value changes. The event `data` will be populated with `mediaTime` and `totalTime`.                                                                                        |
  | `ADS_MANAGER_LOADED`         | Android, iOS | Fires when ads are successfully loaded from the ad servers by the loader. The event `data` will be populated with `adCuePoints`.                                                                            |
  ...
  | `CONTENT_PAUSE_REQUESTED`    | Android, iOS | Fires when content should be paused. This usually happens right before an ad is about to cover the content.                                                                                                 |
  | `CONTENT_RESUME_REQUESTED`   | Android, iOS | Fires when content should be resumed. This usually happens when an ad finishes or collapses.                                                                                                                |
  ...
  | `ICON_TAPPED`                | iOS          | The user has tapped an ad icon. On iOS, the SDK will navigate to the landing page. On tvOS, the SDK will present a modal dialog containing the VAST icon fallback image.                                    |
  | `ICON_FALLBACK_IMAGE_CLOSED` | iOS          | The user has closed the icon fallback image dialog. This may be a good time to resume ad playback, which the SDK autopaused on icon tap. This event only fires for tvOS.                                    |
